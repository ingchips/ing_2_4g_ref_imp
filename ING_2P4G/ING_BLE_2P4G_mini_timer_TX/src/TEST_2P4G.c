...
void ing_2p4g_config_init(void)
{
    ...
    ing_2p4g_config.TimeOut       = 1600;//*6.25us
...
void test_2p4g_timer_init(void)
{
    ...
    TMR_SetReload(APB_TMR2, 0, (TMR_GetClk(APB_TMR2, 0)/1000000)*TEST_TX_INTERVAL);   // 1ms 
...
//        }
        platform_printf("MASTER Rx %d data:", RxPkt111.DataLen);
        ...
        gpio_pluse_num4(3);
//        platform_pplatform_printf("[2G4]Error mode\n");2G4]Event error:%d\n", status);
    }
