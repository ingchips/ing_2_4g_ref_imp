...
{
  while(!(AHB_USB->UsbRControl & (1U<<31))); // AHB Master Idle
...
void USB_SetupDataFifo(void)
{
  ...
  // }
  // addr                += DEP_FSIZ;
  // AHB_USB->UsbIFifo[3] = (addr) | (0 << 16);//in terms of 32-bit words.
  // AHB_USB->UsbIFifo[4] = (addr) | (0 << 16);//in terms of 32-bit words.


  platform_printf("AHB_USB->UsbRFifoSize : %x.\n", AHB_USB->UsbRFifoSize);
  platform_printf("AHB_USB->UsbTFifoSize : %x.\n", AHB_USB->UsbTFifoSize);
  for(i = 0; i < 5; i++)
  {
    platform_printf("AHB_USB->UsbIFifo[%d] : %x.\n", i, AHB_USB->UsbIFifo[i]);
  }
}

void USB_FlushTXFifo(uint8_t FifoNum)
{
  if(!(AHB_USB->UsbIntStat & (0x1 << 6))) // Set Global Non-periodic IN NAK bit
  {
    AHB_USB->UsbDControl |= (0x1 << 7); // Set Global Non-periodic IN NAK
  }

  while(!(AHB_USB->UsbIntStat & (0x1 << 6)));
  while(!(AHB_USB->UsbRControl & (1U << 31))); // AHB Master Idle
  while(AHB_USB->UsbRControl & (0x1 << 5));
  AHB_USB->UsbRControl = ((FifoNum & 0x1F) << 6) | (0x1 << 5); //Flush all the transmit FIFOs  TxFFlush
  while(AHB_USB->UsbRControl & (0x1 << 5));

  AHB_USB->UsbDControl  |=  (0x1 << 8); // Clear Global Non-periodic IN NAK
}

void USB_FlushRXFifo(void)
{
  AHB_USB->UsbDControl  |=  (0x1 << 9); // Set Global OUT NAK 
  while(!(AHB_USB->UsbDControl & (0x1 << 3)));
  AHB_USB->UsbRControl |=  (0x1 << 4);      //RxFFlush
  while(AHB_USB->UsbRControl & (0x1 << 4));
  AHB_USB->UsbDControl  |=  (0x1 << 10); // Clear Global OUT NAK
}

// GAHBCFG.DMAEn=0,DCFG.DescDMA=0 => Slave mode
// GAHBCFG.DMAEn=0,DCFG.DescDMA=1 => Invalid
// GAHBCFG.DMAEn=1,DCFG.DescDMA=0 => Buffered DMA mode   this
...
void USB_DeviceInitialization(void)
{
  ...
  AHB_USB->UsbDConfig |= (3 << 0) | (1 << 24);  // Ena32KHzSusp  PerSchIntvl
  AHB_USB->UsbDConfig &= ~(0x7F << 4);  // DevAddr
  AHB_USB->UsbDConfig &= (~(1 << 23));  // DescDMA 0
...
void USB_ResetAndConfig(void)
{
  ...
  USB_CoreReset();
  // platform_printf("Init 0000 AHB_USB->UsbIntMask : %x\n", AHB_USB->UsbIntMask);

  AHB_USB->UsbAConfig |= (1 << 5) | (3 << 1) | (1 << 0); //DMAEn, 4words
  ...
  // platform_printf("Init 1111 AHB_USB->UsbIntMask : %x\n", AHB_USB->UsbIntMask);
}
    ...
    else
    {
      switch(epNum)
      {
        ...
        default:
          ...
          platform_printf("size : %d, nbPacket : %d, DOSizex : %x.\n", size, nbPacket, AHB_USB->UsbDOxConfig[epNum-1].DOSizex);
        break;
      }
    }
...
uint8_t USB_StartTransfer(uint8_t ep, void *data, uint16_t size, uint32_t flag)
{
    ...
    if(epNum != 0)
    {
      platform_printf("sc_size : %d, sc_nbPacket : %d.\n", sc_size, sc_nbPacket);
    }
...
uint8_t USB_ContinueTransfer(uint8_t ep)
{
    ...
    if(epNum != 0)
    {
      platform_printf("XferSize : %d, sizeTotalLen : %d, sizeRemaining : %d.\n", XferSize, size = transfer->sizeTotalLen, transfer->sizeRemaining);
    }
...
{
    uint8_t epNum = USB_EP_NUM(ep);
    // platform_printf("USB_EnableEp, epNum : %d, type : %d.\n", epNum, type);
    if(USB_IS_EP_DIRECTION_IN(ep))
    {
      if(0 == epNum)
      {
      ...
      {
          platform_printf("g_UsbVar.ep[%d].maxpacket : %d.\n", epNum, g_UsbVar.ep[epNum].maxpacket);
...
USB_ERROR_TYPE_E USB_RecvData(uint8_t ep, void* buffer, uint16_t size, uint32_t flag)
{
    ...
    platform_printf("USB_RecvData, epNum : %d.\n", epNum);
    if(!activeEp){return USB_ERROR_INACTIVE_EP;}
...
void USB_HandleEp0(void)
{
  ...
  switch(g_UsbVar.Ep0State)
  {
    case EP0_IDLE:
    {
      ...
      platform_printf("bmRequestType.Recipient : %d, bRequest : %d, wLength, : %d, event_status : %d.\n", setup->bmRequestType.Recipient, setup->bRequest, setup->wLength, event_status);
    ...
    case EP0_IN_DATA_PHASE:
    {
      if(USB_ContinueTransfer(USB_EP_DIRECTION_IN(0)))
      {
        ...
        USB_EP0StatusOut();
        platform_printf("EP0_IN_DATA_PHASE.\n");
      }
    }
    break;
    case EP0_OUT_DATA_PHASE:
    {
      if(USB_ContinueTransfer(USB_EP_DIRECTION_OUT(0)))
      {
        ...
        g_UsbVar.UserConfig.handler(&event);
        USB_EP0StatusIn();
        platform_printf("EP0_OUT_DATA_PHASE.\n");
      }
    }
    break;
    ...
    case EP0_IN_STATUS_PHASE:
    {
      g_UsbVar.Ep0State = EP0_IDLE;
      USB_GetSetupPacket();
      platform_printf("EP0_IN_STATUS_PHASE.\n");
    }
    break;
    case EP0_OUT_STATUS_PHASE:
    {
      g_UsbVar.Ep0State = EP0_IDLE;
      USB_GetSetupPacket();
      platform_printf("EP0_OUT_STATUS_PHASE.\n");
    }
    break;
    default:
...
uint32_t USB_IrqHandler (void *user_data)
{
  ...
  if(status & (0x1 << 12)) // USB RESET
  {
    ...
    platform_printf("USB RESET=======, UsbIntMask : %x.\n", AHB_USB->UsbIntMask);
  }
  ...
  if(status & (0x1 << 13))
  {
    ...
    platform_printf("USB DEFAULT=======, UsbIntMask : %x.\n", AHB_USB->UsbIntMask);
  }

  if((status & (0x1 << 18)) && (g_UsbVar.DeviceState & (1<<USB_DEVICE_DEFAULT))) //GINTSTS->IEPint 
  {
  ...
  if((status & (0x1 << 19)) && (g_UsbVar.DeviceState & (1<<USB_DEVICE_DEFAULT))) //GINTSTS->OEPint 
  {
    ep_intr = (AHB_USB->UsbDInt) & (AHB_USB->UsbDIntMask);
    uint32_t epnum = 0;
    platform_printf("GINTSTS->OEPint.\n");
    ep_intr = ep_intr >> 16;
    for(epnum = 0; epnum <= DOEP_NUM; epnum++)
    {
      if ((ep_intr >> epnum) & 0x1)
      {
        if(0 == epnum)
        {
          ...
          if(statusEp & (0x1 << 0)) //DOEPINT0->XferCompl
          {
            if(statusEp & (0x1 << 15)) //DOEPINT0->StupPktRcvd
            {
              g_UsbVar.Ep0State = EP0_IDLE;
              platform_printf("0000000=======\n");
              USB_HandleEp0();
            }
            else
            {
          ...
          {
            if((statusEp & (0x1 << 5))&&(g_UsbVar.Ep0State == EP0_OUT_DATA_PHASE)) // DOEPINT0->StsPhseRcvd
            {
              platform_printf("DOEPINT0->StsPhseRcvd\n");
              USB_EP0StatusIn();
            }
            if((statusEp & (0x1 << 3)))// DOEPINT0->SetUp
            {
              platform_printf("DOEPINT0->SetUp\n");
              USB_GetSetupPacket();
            }
          }
        }
        else
        {
          ...
          platform_printf("epnum : %d, GINTSTS->OEPint.\n", epnum);
