...
static void printf_rx_result(uint64_t time, ing2p4g_status_t status, uint8_t *data)
{
    static uint16_t test_cnt = 0;
    static uint16_t ack_cnt = 0;
    static uint16_t miss_cnt = 0;
    ...
    if(status == ING2P4G_SUCCESS){
		...
		if(data[0] == 0)
		{
			...
			GIO_WriteValue(LED_PIN, led_status);
			platform_printf("1\n");
		}
    }else{
        miss_cnt++;
    ...
    if(platform_get_us_time() - tick_start > time)
    {
        tick_start = platform_get_us_time();
        platform_printf("Test %lld time! miss: %d, rev: %d.\r\n", time, miss_cnt, ack_cnt);
        ack_cnt = 0;
        miss_cnt = 0;
    }
}

void opend_24g_rx(void)
{
    ing2p4g_status_t state;
	state = ing2p4g_start_2p4g_rx(slave_tx_len, tx_data);
...
ADDITIONAL_ATTRIBUTE static void EventIrqCallBack(void)
{
	...
	if(status == ING2P4G_SUCCESS){
		if(RxPkt111.Data[0] == 0)
		{
			led_status = !led_status;
			GIO_WriteValue(LED_PIN, led_status);
			platform_printf("LED_PIN : %d\n", led_status);
		}
	}
...
//        else
//        {
//            printf_rx_result(1000000, status, RxPkt111.Data);
//            ing2p4g_start_2p4g_rx(slave_tx_len, tx_data);
//        }
//    }
}
